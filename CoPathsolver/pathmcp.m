...
if (nargin > 4)
  ...
  if (m > 0)
    ...
    idx = find(abs(A)*ones(n,1) == 0);
    if (~isempty(idx))
      for idy = idx',
          if (b(idy) > 0 & t(idy) > 0) | (b(idy) < 0 & t(idy) < 0) ...
                  | (b(idy) == 0 & t(idy) == 0)
              error(['Polyhedral constraints with zero rows and ' ...
                     'infeas rhs']);
          end
      end

      idx = find(abs(A)*ones(n,1));
...
if (status ~= 1) 
  status;
  display('Path fails to solve problem');
  fes = 0;
else fes = 1;
end
